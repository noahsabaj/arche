...
use arche_package::{
    source_tree_digest, Capability, LockDependencyKind, Manifest, PackageNodeId, PortablePath,
    ResolvedGraph, ResolvedPackage, ResolvedSource, Target, Workspace, WorkspaceMember,
};
...
use super::shape::{
    ...
    IntegerType, Mutability, PendingShapeKind, SemanticDeclarationPath,
    SymbolicConstExpression, SymbolicConstNode,
    ...
    SymbolicImpliedCapabilityRequirementSkeleton, SymbolicMethodShapeSkeleton,
    SymbolicCapabilityAccessMode, SymbolicPredicate, SymbolicPredicateShapeSkeleton,
    SymbolicPendingShape,
    SymbolicQueryTermKind, SymbolicQueryTermShapeSkeleton, SymbolicRecordForm,
    SymbolicRecordShapeSkeleton, SymbolicShapeReadiness, SymbolicSourceSpan,
    SymbolicSystemAccessShapeSkeleton, SymbolicTypeShapeSkeleton, SymbolicVariantShapeSkeleton,
...
pub fn check_workspace_c1(
    ...
    for resolved in &graph.packages {
        let member = members.get(&resolved.id).copied().ok_or_else(|| {
            frontend_path_error(
                FrontendErrorCode::Target,
                "TARGET015",
                format!(
                    "C1 source integration requires materialized workspace source for package `{}`",
                    resolved.name
                ),
            )
        })?;
        ...
        for target in member.manifest.targets() {
            let target_id = target_ids.allocate(&member.manifest, &target)?;
            ...
            for module in &target.target.modules {
                for candidate in find_include_input_candidates(&module.ast) {
                    ...
                    uses.push(ResolvedIncludeUse {
                        ...
                        package_root: package.member.directory.clone(),
                        input,
                    });
                }
            }
...
impl NestedBodyCollector<'_, '_, '_> {
    ...
    fn path_with_generic_namespace(
        ...
        let lexical_local = unqualified_path_name(path).and_then(|name| {
            self.scopes
                .iter()
                .rev()
    ...
    while !pending.is_empty() {
        ...
        for pending_import in pending {
            ...
            for resolved in lookup {
                if !visibility_is_subset(&visibility, &resolved.binding.declared_visibility) {
                    return Err(frontend_error(
                        FrontendErrorCode::Visibility,
                        Diagnostic::at(
                            "VISIBILITY004",
...
fn visibility_is_subset(requested: &Visibility, source: &Visibility) -> bool {
    match (requested, source) {
        (_, Visibility::Public) => true,
        (Visibility::Public, _) => false,
        (Visibility::Package, Visibility::Package) => true,
        (Visibility::Package, _) => false,
        (Visibility::DeclaringModule | Visibility::AncestorModule { .. }, Visibility::Package) => {
            true
        }
        (Visibility::DeclaringModule, Visibility::DeclaringModule) => true,
        (
            Visibility::AncestorModule { path: requested },
            Visibility::AncestorModule { path: source },
        ) => requested.starts_with(source),
        (Visibility::DeclaringModule, Visibility::AncestorModule { .. }) => true,
        (Visibility::AncestorModule { .. }, Visibility::DeclaringModule) => false,
    }
}
...
fn validate_final_path_resolution(
    ...
    match &resolution.unresolved {
        None => {
            ...
            if resolution.resolutions.len() > 1
                && !is_import_path_span(target, resolution.span)
            {
...
fn is_import_path_span(target: &ResolvedSymbolicTargetHir, span: Span) -> bool {
    target.modules.iter().any(|module| {
        module.ast.items.iter().any(
            |item| matches!(item, AstItem::Import(import) if import.path.span == span),
        )
    })
}
...
fn has_contextual_self_authority(
    ...
    match &context.source {
        HirItemSource::TraitMethod(_) | HirItemSource::ImplMethod(_) => context.owner.is_some_and(
            |owner| {
...
fn associated_candidate_is_valid(
    ...
    match candidate {
        AssociatedPathCandidate::Item(HirItemRes::Definition(item)) => {
            find_item(packages, item).is_some_and(|item| {
...
fn has_generic_formation_authority(
    ...
    let path_witnesses = item
        ...
        .filter(|path_use| {
            path_has_generic_formation_mismatch(packages, target, path_use, domain)
        })
        .count();
...
fn generic_actual_matches_formal(
    ...
    matches!(
        (actual, formal),
        (
            ResolvedGenericArgument::Type(_),
    ...
    match resolution {
        ...
        Res::Builtin(BuiltinRes {
            ...
            domain != PendingSymbolicDomain::Type
                || path_use.generic_arguments.is_empty()
                || path_use.generic_arguments.iter().any(|argument| {
                    !matches!(argument.value, ResolvedGenericArgument::Type(_))
                })
        }
        Res::Module(_)
        | Res::Item(
...
fn generic_kind_matches_domain(
    ...
    matches!(
        (kind, domain),
        (Some(GenericParameterKind::Type), PendingSymbolicDomain::Type)
            | (
...
fn resolve_symbolic_const(
    ...
    match lower_symbolic_const(context, expression, integer_type) {
        ...
        Err(SymbolicLoweringError::Pending { reason, span }) => Ok(ResolvedSymbolicConst::Pending {
            span,
            reason,
...
fn resolve_symbolic_lifetime(
    ...
    match lower_symbolic_lifetime(context, Some(lifetime), span) {
        ...
        Err(SymbolicLoweringError::Pending { reason, span }) => Ok(ResolvedSymbolicLifetime::Pending {
            span,
            reason,
            canonical: format!("'{}", lifetime.as_str()),
        }),
...
fn resolve_symbolic_effect(
    ...
    match lowered {
        ...
        Err(SymbolicLoweringError::Pending { reason, span }) => Ok(ResolvedSymbolicEffect::Pending {
            span,
            reason,
            canonical,
        }),
...
fn lower_declaration_shape_skeleton(
    ...
    let payload = match &item.source {
        HirItemSource::Declaration(declaration) => match &declaration.kind {
            ...
            AstDeclarationKind::Component(record) | AstDeclarationKind::Resource(record) => {
                SymbolicDeclarationPayloadSkeleton::Record(lower_record_shape(
                    ...
                    record.fields.iter().map(|field| (Some(&field.name), &field.ty)),
                )?)
            }
            ...
            AstDeclarationKind::Function(function) => {
                SymbolicDeclarationPayloadSkeleton::Callable(lower_callable_shape(
                    context,
            ...
            AstDeclarationKind::Generator(generator) => {
                SymbolicDeclarationPayloadSkeleton::Callable(lower_generator_callable_shape(
                    context, generator,
                )?)
            }
            AstDeclarationKind::System(system) => {
        ...
        HirItemSource::TraitMethod(method) => SymbolicDeclarationPayloadSkeleton::Callable(
            lower_method_callable_shape(context, &method.signature)?,
        ),
    ...
    if let Some(canonical) = super::shape::try_canonicalize_declaration_shape(&skeleton)
        .map_err(shape_error)?
    {
...
fn lower_definition_owner_entry(
    ...
    let skeleton = match &item.source {
        ...
        | HirItemSource::ImplMethod(_)
        | HirItemSource::QueryParameter { .. } => {
            Ok(SymbolicDefinitionOwnerSkeleton::TopLevel)
        }
    }?;
    if let Some(canonical) = super::shape::try_canonicalize_definition_owner(&skeleton)
        .map_err(shape_error)?
    {
...
fn lower_effect_sets(
    ...
    let throws = effects
        ...
        .collect::<Result<Vec<_>, FrontendError>>()?;
    reject_duplicate_source_effects(&requires, "requires")?;
    reject_duplicate_source_effects(&throws, "throws")?;
    Ok(SymbolicEffectSetsSkeleton {
        requires: requires.into_iter().map(|(effect, _)| effect).collect(),
        throws: throws.into_iter().map(|(effect, _)| effect).collect(),
    })
}

fn reject_duplicate_source_effects(
    rows: &[(SymbolicEffectShapeSkeleton, Span)],
    set_name: &str,
) -> Result<(), FrontendError> {
    let mut first = BTreeMap::new();
    for (effect, span) in rows {
        if let Some(previous) = first.insert(effect.clone(), *span) {
            return Err(frontend_error(
                FrontendErrorCode::Target,
                Diagnostic::at(
                    "EFFECT001",
                    *span,
                    format!("duplicate member in source `{set_name}` effect set"),
                )
                .with_secondary(previous, "first declared here"),
            ));
        }
...
fn lower_effect_shape(
    ...
    Ok(match resolve_symbolic_effect(context, effect)? {
        ResolvedSymbolicEffect::Resolved(effect) => {
            SymbolicEffectShapeSkeleton::resolved(effect.ty)
        }
...
fn encode_alpha_symbolic_shape(shape: &ResolvedSymbolicShape) -> Result<Vec<u8>, FrontendError> {
    use super::shape::{encode_symbolic_const, encode_symbolic_effect, encode_symbolic_type};
    ...
    push_resolution_list(&mut output, &shape.types, |value| match value {
        ResolvedSymbolicType::Resolved(ty) => encode_symbolic_type(ty).map_err(shape_error),
        ResolvedSymbolicType::Pending {
            reason, canonical, ..
    ...
    push_resolution_list(&mut output, &shape.effects, |value| match value {
        ResolvedSymbolicEffect::Resolved(value) => {
            encode_symbolic_effect(value).map_err(shape_error)
        }
...
fn trait_owner_from_path(
    ...
    let Some(owner) = embedded_core.lookup_prelude_definition(name.as_str(), VirtualNamespace::Type)
    else {
        return Ok(None);
    };
    Ok(embedded_core
...
fn generic_associated_candidates(
    ...
    for trait_path in trait_paths {
        if let Some(trait_owner) = trait_owner_from_path(
            packages,
            graph,
            owner_item,
            &trait_path,
...
fn contextual_self_candidates(
    ...
    Ok(owner.map_or_else(Vec::new, |owner| {
        visible_child_method_candidates(packages, owner, member, from)
    }))
}
...
fn resolve_associated_path(
        ...
        && owner_path.segments.is_empty()
    {
        ...
        if lexical.locals.contains_key(name.as_str()) {
            return Err(associated_name_error(
                path,
                "a lexical value cannot be an associated-path owner",
            ));
        }
        if let Some(parameter) = lexical.generics.get(name.as_str()).copied() {
            (
                AssociatedPathOwner::Generic(parameter),
            ...
            if nominal.len() == 1 {
                ...
                let Some(embedded_owner) = embedded_core
                    .lookup_prelude_definition(name.as_str(), VirtualNamespace::Type)
                else {
                    return Ok(None);
                };
                let mut direct = Vec::new();
                if let Some(variant) = embedded_core
                    .lookup_enum_variant(embedded_owner, member.name.as_str())
                {
    ...
    Ok(Some(AssociatedLookup::Pending(
        AssociatedPathResolution {
            owner,
...
fn build_inventory(
    ...
    for package in packages {
        ...
        entries.push(package.member.manifest.source_entry.clone());
    ...
    for package in packages {
        ...
        let budgets = package.member.manifest.const_eval;
    ...
    match &argument.value {
        ResolvedGenericArgument::Type(value) => {
            ...
            write_resolved_symbolic_value(output, value, |value| {
                super::shape::encode_symbolic_type(value).map_err(shape_error)
            })?;
            output.push(')');
        }
        ResolvedGenericArgument::Lifetime(value) => {
...
mod tests {
    ...
    use super::*;
    use crate::try_canonicalize_declaration_shape;
    ...
    fn both_corpora_hir_text_is_byte_exact_and_host_path_free() {
        ...
        assert_eq!(
            digest,
            "880BCF973A672B7C51D9F80DE7DBA1FE60C3F8D3D2BD9846C0B3F54CA04459B0"
        );
    }
    ...
    fn accepted_nested_and_type_const_bodies_are_all_addressable() {
        let cases = [
            BodyCorpusExpectation {
                ...
                count: 72,
                kind_counts: &[
                    (SemanticBodyKind::Declaration, 49),
                    ...
                    (SemanticBodyKind::IntegerGenericArgument, 6),
                ],
            },
            BodyCorpusExpectation {
                ...
                count: 43,
                kind_counts: &[
                    (SemanticBodyKind::Declaration, 30),
                    (SemanticBodyKind::WorldInitializer, 1),
        ...
        let constructor = root
            ...
            .find(|binding| {
                binding.name == "Wrapper"
                    ...
                    && matches!(
                        binding.target,
                        SemanticBindingTarget::NominalConstructor(_)
                    )
            })
            .unwrap();
    ...
    fn system_capability_access_tag_distinguishes_shared_and_mutable_references() {
        ...
        assert_eq!(shared_implied[0].access, SymbolicCapabilityAccessMode::Shared);
        assert_eq!(mutable_implied[0].access, SymbolicCapabilityAccessMode::Mutable);
        ...
        let duplicate = inline_library_result(
            "pub system Duplicate(capability: &Udp) requires {Udp, Udp} throws {} {}\n",
        )
        .unwrap_err();
        assert_eq!(duplicate.diagnostic.code, "EFFECT001");
        assert_eq!(duplicate.diagnostic.secondary.len(), 1);
    }
    ...
    fn user_associated_paths_retain_typed_owner_spans_and_candidates() {
        ...
        let trait_method = target
            ...
            .iter()
            .find(|candidate| {
                candidate.owner == Some(factory.id)
                    && candidate.name.as_deref() == Some("make")
            })
            .unwrap();
        ...
        assert_eq!(associated.path_span, resolution("call", "Wrapper::make").span);
        ...
        assert_eq!(associated.member_span, via_self.path_uses[0].path.segments[0].span);
    ...
    fn embedded_qualified_members_and_record_constructors_resolve_exactly() {
        ...
        assert!(target.path_resolutions.iter().all(|resolution| {
            resolution.unresolved != Some(UnresolvedPathKind::UnknownName)
        }));
    }
