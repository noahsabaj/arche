...
pub enum SymbolicType {
    ...
    FunctionPointer {
        ...
        requires: Vec<SymbolicType>,
        throws: Vec<SymbolicType>,
    },
    BoundType {
        depth: u64,
        index: u64,
...
impl Default for SymbolicDefinitionOwnerSkeleton {
    fn default() -> Self {
        Self::TopLevel
    }
}
...
pub fn symbolic_const_readiness(
    expression: &SymbolicConstExpression,
) -> SymbolicShapeReadiness {
...
pub fn symbolic_type_readiness(ty: &SymbolicType) -> SymbolicShapeReadiness {
    let type_list = |types: &[SymbolicType]| {
        types.iter().fold(
            SymbolicShapeReadiness::ConstIndependent,
            |readiness, ty| readiness.combine(symbolic_type_readiness(ty)),
        )
    };
    ...
    match ty {
        SymbolicType::Slice(element) | SymbolicType::RawPointer { pointee: element, .. } => {
        ...
        SymbolicType::Tuple(types) => type_list(types),
        ...
        SymbolicType::FunctionPointer {
            ...
            .combine(type_list(requires))
            .combine(type_list(throws)),
        SymbolicType::Closure {
            ...
            .iter()
            .fold(SymbolicShapeReadiness::ConstIndependent, |state, capture| {
                state.combine(symbolic_type_readiness(&capture.ty))
            })
            .combine(type_list(parameters))
            .combine(symbolic_type_readiness(result))
            .combine(type_list(requires))
            .combine(type_list(throws))
        ...
        SymbolicType::JoinHandle { result, throws } => {
            symbolic_type_readiness(result).combine(type_list(throws))
        }
...
fn generator_target_readiness(target: &GeneratorTarget) -> SymbolicShapeReadiness {
    let arguments = match target {
        GeneratorTarget::Named { arguments, .. }
        | GeneratorTarget::Anonymous { arguments, .. } => arguments,
    };
    arguments.iter().fold(
        SymbolicShapeReadiness::ConstIndependent,
        |state, argument| {
...
pub fn symbolic_predicate_readiness(
    ...
    match predicate {
        SymbolicPredicate::Trait {
            ...
            arguments,
            ..
        } => arguments.iter().fold(symbolic_type_readiness(self_type), |state, argument| {
            state.combine(match argument {
                GenericArgumentShape::Type(ty) => symbolic_type_readiness(ty),
                GenericArgumentShape::IntegerConst(value) => symbolic_const_readiness(value),
                GenericArgumentShape::Lifetime(_) => SymbolicShapeReadiness::ConstIndependent,
...
fn validate_effect_sets(
    ...
    for set in [&effects.requires, &effects.throws] {
        let mut resolved = Vec::new();
        for effect in set {
            *readiness = readiness.combine(validate_effect_leaf(effect)?);
...
fn validate_record(
    ...
    if record.form == SymbolicRecordForm::Record {
        validate_unique_member_names(
            record.fields.iter().filter_map(|field| field.name.as_deref()),
...
fn encode_predicate_skeleton_set(
    ...
    let values = predicates
        ...
        .map(|predicate| match predicate {
            SymbolicPredicateShapeSkeleton::Resolved { value, .. } => Ok(value.clone()),
            SymbolicPredicateShapeSkeleton::Pending(_) => Err(
...
fn encode_effect_skeleton_set(
    ...
    let values = effects
        ...
        .map(|effect| match effect {
            ...
            SymbolicEffectShapeSkeleton::Pending(_) => Err(
                ShapeEncodingError::InvalidDeclarationShape(
...
fn encode_debug_effect_set(
    ...
    count(output, rows.len(), "C1 effect skeleton set")?;
    for row in rows {
...
fn encode_debug_callable(
    ...
    count(output, callable.parameters.len(), "C1 callable parameter list")?;
    for parameter in &callable.parameters {
        output.push(parameter.mode.tag());
...
fn encode_type_into(ty: &SymbolicType, output: &mut Vec<u8>) -> Result<(), ShapeEncodingError> {
    match ty {
        ...
        SymbolicType::FunctionPointer {
            ...
            type_set(output, requires, "requires set")?;
            type_set(output, throws, "throws set")?;
        }
        ...
        SymbolicType::Closure {
            ...
            nested_type(output, result)?;
            type_set(output, requires, "closure requires")?;
            type_set(output, throws, "closure throws")?;
            generic_arguments(output, arguments)?;
        }
        SymbolicType::Generator {
            target,
            ...
            nested_type(output, result)?;
            type_set(output, requires, "generator requires")?;
            type_set(output, throws, "generator throws")?;
        }
        SymbolicType::JoinHandle { result, throws } => {
            output.push(30);
            nested_type(output, result)?;
            type_set(output, throws, "join handle throws")?;
        }
        SymbolicType::GeneratorFactory {
            target,
            captures,
...
mod tests {
    ...
    fn anonymous_state_shapes_reject_provisional_zero_ordinals() {
        let ty = SymbolicType::GeneratorFactory {
            target: GeneratorTarget::Anonymous {
    ...
    fn callable_modes_and_effect_sets_match_manual_vector() {
        ...
        let shape = SymbolicDeclarationShapeSkeleton {
            ...
            predicates: Vec::new(),
            payload: SymbolicDeclarationPayloadSkeleton::Callable(
    ...
    fn trait_and_impl_entries_reuse_exact_child_callable_shape() {
        let child = declaration_shape(SymbolicDeclarationPayloadSkeleton::Callable(
            SymbolicCallableShapeSkeleton {
                kind: SymbolicCallableKind::Function,
                parameters: vec![SymbolicCallableParameterSkeleton {
    ...
    fn ready_wrapper_rejects_duplicate_structural_members() {
        ...
        let duplicate_variants = declaration_shape(SymbolicDeclarationPayloadSkeleton::Enum(
            vec![
                SymbolicVariantShapeSkeleton {
                    name: "Same".to_owned(),
                    form: SymbolicRecordForm::Unit,
        ...
        let duplicate_variant_fields = declaration_shape(
            SymbolicDeclarationPayloadSkeleton::Enum(vec![SymbolicVariantShapeSkeleton {
                name: "Record".to_owned(),
                form: SymbolicRecordForm::Record,
                fields: vec![field("same"), field("same")],
            }]),
        ...
        let callable = || {
            declaration_shape(SymbolicDeclarationPayloadSkeleton::Callable(
                SymbolicCallableShapeSkeleton {
                    kind: SymbolicCallableKind::Function,
                    parameters: Vec::new(),
                    result: type_leaf(SymbolicType::Unit),
    ...
    fn declaration_predicate_and_effect_sets_sort_and_reject_duplicates() {
        let first = SymbolicPredicateShapeSkeleton::resolved(
            SymbolicPredicate::LifetimeOutlives {
                longer: SymbolicLifetime::Static,
                shorter: SymbolicLifetime::Bound { depth: 0, index: 0 },
            },
        ...
        assert_eq!(
            canonical_declaration_bytes(&effect_shape(vec![
                i32_effect.clone(),
                u8_effect.clone()
            ])),
            canonical_declaration_bytes(&effect_shape(vec![
        ...
        assert_eq!(
            try_canonicalize_declaration_shape(&effect_shape(vec![
                i32_effect.clone(),
                i32_effect
            ])),
            Err(ShapeEncodingError::DuplicateEffect)
