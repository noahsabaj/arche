...
mod tests {
    ...
    fn push_golden_text(output: &mut Vec<u8>, text: &str) {
        output.extend_from_slice(&u64::try_from(text.len()).unwrap().to_le_bytes());
    ...
    fn push_golden_diagnostic(output: &mut Vec<u8>, case: &str, error: &Diagnostic) {
        push_golden_text(output, case);
        push_golden_text(output, error.code);
        push_golden_text(output, &error.message);
        push_golden_text(output, &error.primary.message);
    ...
    #[test]
    fn every_frozen_keyword_is_rejected_in_an_identifier_position_exactly() {
        const KEYWORDS: &str = concat!(
            "as break catch commands component const continue default else enum entity false ",
            "fn for gen if impl in init let loop match mod move mut package pub query read ref ",
            "requires resource resume return run schedule Self self spawn static str struct super ",
            "system tag throw throws trait true type unsafe use where while world yield yields ",
            "i8 i16 i32 i64 u8 u16 u32 u64 isize usize f32 f64 bool char",
        );
        let keywords = KEYWORDS.split_ascii_whitespace().collect::<Vec<_>>();
        assert_eq!(
            keywords.len(),
            71,
            "the frozen keyword matrix is incomplete"
        );
        for keyword in keywords {
            let source = format!("pub struct {keyword};");
            let error = parse(&source).unwrap_err();
            assert_eq!(error.code, "PARSE001", "reserved keyword `{keyword}`");
            assert_eq!(
                error.message, "expected struct name",
                "reserved keyword `{keyword}`"
            );
            assert_eq!(
                error.primary.message, error.message,
                "reserved keyword `{keyword}`"
            );
            let start = 11_u64;
            let end = start + u64::try_from(keyword.len()).unwrap();
            assert_eq!(
                error.primary.span,
                Some(Span {
                    file: FileId(41),
            ...
            assert!(
                ...
                "reserved keyword `{keyword}` gained secondary labels"
            );
            assert!(
                error.notes.is_empty(),
                "reserved keyword `{keyword}` gained ordered notes"
            );
        }
    }
    ...
    fn rejects_all_frozen_empty_comma_list_forms() {
        ...
        let mut golden = b"ARCHE-C1-EMPTY-COMMA-DIAGNOSTICS\0".to_vec();
        for (name, source) in cases {
            let error = parse(source).unwrap_err();
            assert_eq!(error.code, "PARSE001", "{name}");
            let comma = u64::try_from(source.find(',').unwrap()).unwrap();
            assert_eq!(
                error.primary.span,
                Some(Span {
                    file: FileId(41),
                ...
                "{name} must diagnose its exact empty-list comma"
            );
            assert!(error.secondary.is_empty(), "{name}");
            assert!(error.notes.is_empty(), "{name}");
            push_golden_diagnostic(&mut golden, name, &error);
        }
        let digest: [u8; 32] = Sha256::digest(&golden).into();
        assert_eq!(
            digest,
            [
                0x21, 0xe7, 0x47, 0x48, 0x38, 0xd6, 0x89, 0xc9, 0xbe, 0x1b, 0x78, 0xab, 0x50, 0x56,
                0xf1, 0xfe, 0x8b, 0x5c, 0x34, 0xa8, 0x4e, 0xb3, 0x7b, 0x68, 0xe3, 0x8a, 0x8b, 0x65,
                0xe7, 0xab, 0x7b, 0x11,
            ]
        );
    }
    ...
    fn rejects_singleton_and_multi_element_tuple_double_commas() {
        ...
        let mut golden = b"ARCHE-C1-TUPLE-DOUBLE-COMMA-DIAGNOSTICS\0".to_vec();
        for source in cases {
            let error = parse(source).unwrap_err();
            assert_eq!(error.code, "PARSE001", "{source}");
            let comma = u64::try_from(source.find(",,").unwrap() + 1).unwrap();
            assert_eq!(
                error.primary.span,
                Some(Span {
                    file: FileId(41),
                ...
                "{source} must diagnose its exact second comma"
            );
            assert!(error.secondary.is_empty(), "{source}");
            assert!(error.notes.is_empty(), "{source}");
        ...
        assert_eq!(
            digest,
            [
                0x33, 0x1d, 0xb5, 0x6a, 0xf0, 0x44, 0x57, 0x90, 0x9d, 0x1d, 0xe4, 0x36, 0x08, 0x29,
                0x45, 0x06, 0x16, 0x12, 0xe1, 0x49, 0xad, 0x34, 0xa8, 0xc5, 0xa0, 0x18, 0x98, 0xb6,
                0x24, 0x3c, 0x31, 0xfc,
            ]
        );
    }
    ...
    #[test]
    fn active_parser_negative_matrix_has_exact_codes_spans_messages_and_note_order() {
        let cases = [
            (
                "method generics require turbofish",
                "pub fn f(v: V) { v.method<T>(); }",
                "PARSE001",
            ),
            (
                "cast comparison requires grouping",
                "pub fn f(value: T, rhs: T) { value as T < rhs; }",
                "PARSE001",
            ),
            (
                "commands parameter name",
                "pub system S(commands: commands) {}",
                "PARSE001",
            ),
            ("commands function name", "pub fn commands() {}", "PARSE001"),
            (
                "commands local name",
                "pub fn f() { let commands = 1i32; }",
                "PARSE001",
            ),
            ("commands type", "pub type T = commands;", "PARSE001"),
            ("commands import root", "use commands::item;", "PARSE001"),
            (
                "commands const expression",
                "pub const C: i32 = commands;",
                "PARSE001",
            ),
            (
                "visibility bare dependency",
                "pub(in dependency) struct S;",
                "PARSE001",
            ),
            (
                "visibility dependency path",
                "pub(in dependency::module) struct S;",
                "PARSE001",
            ),
            ("visibility Self", "pub(in Self) struct S;", "PARSE001"),
            (
            ...
            (
                "Self associated bound",
                "pub struct S<T> where T: Self::Assoc {}",
                "PARSE001",
            ),
            (
                "Self associated requires",
                "pub fn f() requires { Self::Assoc } {}",
                "PARSE001",
            ),
            (
                "Self associated schedule",
                "pub schedule S { run Self::Assoc; }",
                "PARSE001",
            ),
            (
                "Self associated const",
                "pub type A = [u8; Self::Assoc];",
                "PARSE001",
            ),
            (
                "closure ambiguous typed alternative",
                "pub fn f() { let invalid = |a | b: T| a; }",
                "PARSE001",
            ),
            (
                "closure ambiguous referenced alternative",
                "pub fn f() { let invalid = |&a | b: T| a; }",
                "PARSE001",
            ),
            (
                "generator closure ambiguous alternative",
                "pub fn f() { let invalid = gen |a | b: T| resume () yields () a; }",
                "PARSE001",
            ),
            ("module semicolon", "mod child", "PARSE003"),
            (
                "let semicolon",
                "pub fn f() { let x = 1i32 let y = 2i32; }",
                "PARSE001",
            ...
            (
                "world spawn semicolon",
                "pub world W { init { spawn {} } }",
                "PARSE001",
            ),
            (
                "schedule run semicolon",
                "pub schedule S { run package::m::system }",
                "PARSE001",
            ),
        ];
        let mut golden = b"ARCHE-C1-ACTIVE-PARSER-NEGATIVE-MATRIX\0".to_vec();
        for (name, source, code) in cases {
            let error = parse(source).unwrap_err();
            assert_eq!(error.code, code, "{name}");
            assert!(error.primary.span.is_some(), "{name}");
            assert_eq!(error.primary.message, error.message, "{name}");
            assert!(error.secondary.is_empty(), "{name}");
            assert!(error.notes.is_empty(), "{name}");
            push_golden_diagnostic(&mut golden, name, &error);
        }
        ...
        let root = fixture_root().join("vectors");
        for (name, code) in [
            ("invalid-empty-comma.arc", "PARSE001"),
            ("invalid-number-member.arc", "LEX002"),
            ("invalid-reserved-identifier.arc", "PARSE001"),
            ("invalid-tuple-double-comma.arc", "PARSE001"),
        ] {
            let bytes = fs::read(root.join(name)).unwrap();
            let error = parse_reader(FileId(11), Cursor::new(bytes)).unwrap_err();
            assert_eq!(error.code, code, "{name}");
            assert!(error.primary.span.is_some(), "{name}");
            assert_eq!(error.primary.message, error.message, "{name}");
            assert!(error.secondary.is_empty(), "{name}");
        ...
        assert_eq!(
            digest,
            [
                0xe1, 0x65, 0xc9, 0x77, 0x5a, 0x20, 0xab, 0x95, 0x0b, 0x9a, 0x5f, 0x2a, 0xad, 0x3d,
                0x25, 0xc0, 0x72, 0xbc, 0xf2, 0x4c, 0x38, 0x61, 0x75, 0xd6, 0xfc, 0xf8, 0x36, 0x5d,
                0x61, 0xe5, 0xfb, 0x5e,
            ]
        );
    }
