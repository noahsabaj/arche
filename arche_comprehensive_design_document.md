...
registry-snapshot commitment version `2` includes all six span coordinates.
Before commitment validation or PackageNodeId allocation, the span must be a
nonempty single-line range with one-based line/column endpoints, a column width
of at least the nine scalars in `[package]`, and a UTF-8 byte width no smaller
than that scalar width; otherwise it is `DEPENDENCY003`. The canonical virtual
registry-manifest label is the exact UTF-8 concatenation `registry/`, the
canonical package-name bytes, `/`, the canonical SemVer release without build
metadata, and `/Arche.toml`; for example,
`registry/example/math/1.2.3/Arche.toml`. PackageNodeId exhaustion retains that
label and all six committed coordinates rather than substituting a workspace or
dependency span. Source acquisition later revalidates the range, its UTF-8
boundaries, coordinates, and `[package]` header bytes against the included
manifest through the assigned package-cache adapter before any semantic row can
be branded.
