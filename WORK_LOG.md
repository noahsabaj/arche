...
- The next missing link is generalizing the native ECS table model beyond the current fixture-specific decoded table path.
...
|---|---|---|
| M21-001 | Define reusable native ECS table model | Native table state has a reusable model for descriptor, startup, schedule, and query-plan records. |
...
| Subproblem | Confidence |
|---|---:|
| M20-005 executes `move_system` through decoded native descriptor/startup/schedule/query-plan tables only | 98/100 |
| `tools/test.ps1` includes `executes_move_system_from_decoded_native_ecs_tables` while keeping valid `move_system` exit `47` and existing generated-binary corruption checks | 99/100 |
...
| Board state moves M20-005 to Done, marks M20 complete, and promotes M21-001 as the next proof | 98/100 |
| Controlled M21 backlog is limited to native ECS table generalization proofs | 97/100 |

Weighted confidence: 98/100.

Verification pass:

- The active board has only `M21-001` in `Ready`.
- `Doing` is empty.
- `Backlog` contains M21-002 through M21-005 only.
- `Done` contains completed M0, completed M1, completed M2, completed M3, completed M4, completed M5, completed M6, completed M7, completed M8, completed M9, completed M10, completed M11, completed M12, completed M13, completed M14, completed M15, completed M16, completed M17, completed M18, completed M19, and completed M20.
