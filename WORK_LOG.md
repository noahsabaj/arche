...
- The next missing link is native startup operation table materialization from metadata.
...
- Generated native binaries can carry complete decoded `ARCHEECS` metadata, and native startup can validate its envelope, register descriptor counts, materialize descriptor record offsets/lengths, decode component/resource/system/query/schedule descriptor records, dispatch resource/spawn/run startup operation kinds through controlled handlers, apply the first resource payload, create one spawn row, dispatch `run Demo.Main`, materialize compiled schedule state, build explicit query-planning state, and execute compiled `Demo.Move` query-loop code through named stack-resident execution state.
...
- System declarations, query metadata, Core query-loop bodies, compiled native `Demo.Move` row scan/math/store code, native query-planning state, compiled schedule state, and a named native execution-state layout exist, but startup operations are still handled by straight-line offset checks/handlers rather than a decoded native startup operation table, and query planning is still fixture-specific rather than driven from decoded descriptor records.
...
| Subproblem | Confidence |
|---|---:|
| M20-002 decodes system/query/schedule numeric descriptor fields into explicit native descriptor-table slots only | 99/100 |
| `tools/test.ps1` includes `decodes_native_system_query_schedule_descriptor_records` plus generated-binary descriptor corruption checks while keeping valid `move_system` exit `47` | 99/100 |
...
| Board state moves M20-002 to Done and promotes M20-003 as the next proof | 98/100 |
| Controlled M20 backlog remains limited to startup operation tables and table-driven query execution | 97/100 |

Weighted confidence: 98/100.

Verification pass:

- The active board has only `M20-003` in `Ready`.
- `Doing` is empty.
- `Backlog` contains M20-004 through M20-005 only.
- `Done` contains completed M0, completed M1, completed M2, completed M3, completed M4, completed M5, completed M6, completed M7, completed M8, completed M9, completed M10, completed M11, completed M12, completed M13, completed M14, completed M15, completed M16, completed M17, completed M18, completed M19, M20-001, and M20-002.
