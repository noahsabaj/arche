...
Review and local exact-head evidence on `f2c6345438acaddd89b36527c78bf890c079cdef`:

- Adversarial scope, effect-boundary, contextual-`Self`, namespace, provenance, deterministic-encoding, and cross-host-byte reviews found no remaining correctness, safety, determinism, contract, integration, or mandatory-coverage blocker.
- Formatting, locked workspace/all-target checking, strict `-D warnings` Clippy, diff, and checkout status passed. Locked debug and release suites each passed `452` tests with `10` explicit WSL-only ignores; the active per-binary pass counts were `7, 0, 10, 11, 179, 42, 52, 151`. Duplicate-dependency output was empty. Cargo Audit 0.22.2 loaded `1,211` advisories, scanned `48` locked dependencies, and found no advisory.
- PowerShell Core 7.6.4 and Windows PowerShell 5.1 each reported `105` PASS, `2` documented host SKIPs, and `25` WSL executions with exit `0`: `210` PASS, `4` skip records, and `50` WSL executions combined.
- Initial hosted run [31524114815](https://github.com/noahsabaj/arche/actions/runs/31524114815) exposed three Windows-CRLF versus Git/LF golden mismatches. Commit `f2c6345438acaddd89b36527c78bf890c079cdef` pins C1 source/text fixtures to LF and `.bin` inputs as binary, then derives the affected goldens from the committed bytes. The existing checkout passed all `179` frontend tests and strict frontend Clippy; a fresh detached checkout independently passed all `179` frontend tests with byte-for-byte Git-blob agreement.

Protected implementation and merged-main evidence:

- Pull request [#16](https://github.com/noahsabaj/arche/pull/16) published implementation commits `7faf43a96d0e4ee4c45fb0028df0a305ffa4f5d6` and `f2c6345438acaddd89b36527c78bf890c079cdef`. Exact implementation-head run [31525196910](https://github.com/noahsabaj/arche/actions/runs/31525196910) passed [Native Linux `93891673949`](https://github.com/noahsabaj/arche/actions/runs/31525196910/job/93891673949), [Windows `93891673899`](https://github.com/noahsabaj/arche/actions/runs/31525196910/job/93891673899), [physical greater-than-4-GiB source `93891674016`](https://github.com/noahsabaj/arche/actions/runs/31525196910/job/93891674016), and [sparse greater-than-4-GiB executable `93891673983`](https://github.com/noahsabaj/arche/actions/runs/31525196910/job/93891673983).
- Pull request #16 merged as `00e0fd93b5c8d57f97071a1f48939366e62e6820`. Exact merged-main run [31526456910](https://github.com/noahsabaj/arche/actions/runs/31526456910) passed [Native Linux `93895877536`](https://github.com/noahsabaj/arche/actions/runs/31526456910/job/93895877536), [Windows `93895877564`](https://github.com/noahsabaj/arche/actions/runs/31526456910/job/93895877564), [physical greater-than-4-GiB source `93895877544`](https://github.com/noahsabaj/arche/actions/runs/31526456910/job/93895877544), and [sparse greater-than-4-GiB executable `93895877568`](https://github.com/noahsabaj/arche/actions/runs/31526456910/job/93895877568).

The evidence-only documentation commit containing this closure record must pass the same four protected contexts before merging. It records the already-merged C1 boundary and advances only M27-C's internal active slice from C1 to C2; it does not close M27-C or add later-gate semantics.
...
Pull request #15 merged predecessor implementation commit `7a0357563c02ae9e6c1fc7caea9444b6593380de` and evidence-only commit `5f4af1a77153657977b3361b24e2a4a5f566dd53` as `78901abe643a15b7550a2c8b690003c45e190bb8`. Exact documentation-inclusive PR-head run [31515173282](https://github.com/noahsabaj/arche/actions/runs/31515173282) and exact merged-main run [31515711289](https://github.com/noahsabaj/arche/actions/runs/31515711289) each passed Native Linux, Windows, physical greater-than-4-GiB source, and sparse greater-than-4-GiB executable. M27-C1 resumed from that merged predecessor authority.
...
M25 answers its binary question **yes**: Arche executes Arena from verified semantics, descriptor identities, checked layouts, startup operations, and query bindings; no Arena or Demo identity selects production storage or execution. This proves the descriptor-generic supported shape, not arbitrary Core execution.

Five ordered implementation commits establish the proof:

- `c7032f4` derives canonical, checked `NativeWorldStoragePlan` values from verified Core schemas and runtime descriptors.
- `ab82e06` emits descriptor-sized, multi-table stack storage and catalogs with checked alignment.
- `064ed9f` carries typed `i32` startup literals end to end and materializes arbitrary component lists transactionally.
- `50585d4` matches archetype subsets and binds query terms by stable component ID.
- `2be3c51` derives one `VerifiedCoreExecutionShape` and uses it for reference and native execution, with a test-only `ARCHEOBS1` observer that compares live post-schedule state.

Local closure evidence on `2be3c51`:

- Formatting, locked debug and release all-target suites, normal Clippy, both complete PowerShell runners, `cargo audit`, and `git diff --check` passed. Debug and release each ran 146 tests with no failures; three e2e scripts were discovered.
- Strict Clippy reproduced exactly five known diagnostics in the existing `too_many_arguments`, `type_complexity`, and `filter_map_bool_then` categories—down from the audit's eight—with no new diagnostic or category.
- Both PowerShell Core 7.6.3 and Windows PowerShell 5.1 generated and ran Arena through WSL. Its descriptor-derived production frame was 1,360 bytes and it exited `47`.
- Independent observed native/reference streams were byte-identical. Demo positions became `(4,6)` and `(11,22)`. Arena Vitality became `(11,102)`, `(22,203)`, and `(33,304)` while excluded rows `(40,400)` and `(50,500)`, Regeneration payloads, Faction values `1..5`, memberships, row counts, and capacities `4`/`2` remained unchanged.
- Required implementation-head CI run [`29291558560`](https://github.com/noahsabaj/arche/actions/runs/29291558560) passed. `Proof / Native Linux` job [`86956066315`](https://github.com/noahsabaj/arche/actions/runs/29291558560/job/86956066315) ran the native observer, published fixtures, Unix alias and executable-permission gates, dependency audit, and tracked-diff check. `Proof / Windows` job [`86956066332`](https://github.com/noahsabaj/arche/actions/runs/29291558560/job/86956066332) passed both PowerShell editions and its tracked-diff check.
- Every phase preserved repository `AUDIT.md` SHA-256 `B5A84B5DA0E28106A220D0A05F0AC1B54720D955DEA98608376E51BCBB41C48E` and external ledger SHA-256 `0F82FFA0178BEC66298FE535A75A8824B3501DB7DA919B430BCCA7762B25A2A7`. Tooling changed no repository file; `cargo audit` refreshed only the user-level advisory cache.
...
none blocks the in-repository M27-C implementation work.
...
| Issue | Title | Notes |
|---|---|---|
| M27-C | General language semantics and verified generic Core | C0 and C1 are Done. C2 is Doing: implement const-independent type checking, explicit conversions, symbolic type-level-const obligations, static trait selection, orphan/coherence and `impl default`, operator dispatch, and exhaustive pattern decision trees. Then C3-C6 implement MIR/calls/ownership; effects/stateful abstractions and semantic IDs; verified Core/CTFE/interface closure; and public closure without advancing D early. Preserve the package-aware M27-B authority and keep instance/layout/AOT concerns in D/G. |

M27-C internal progress: C0-C1 **Done**; C2 **Doing**; C3-C6 **Backlog**. These are
...
|---|---|---|
| M27-C1 | Implement complete retained syntax and symbolic HIR/type shapes | Immutable retained module/include snapshots now feed the syntax-complete incremental lexer and parser, complete AST bodies, package-aware resolved symbolic HIR, branded embedded-Core authority, checked session arenas, and deterministic unverified inventory/type/generic-shape encoders. Both mandatory workspaces prove every accepted body and selected shape without reopening source or minting provisional stable IDs; C2-C5 semantic authority remains explicitly pending. Local debug and release suites each passed `452` tests with `10` WSL-only ignores; strict Clippy, both complete PowerShell editions, duplicate-dependency, RustSec, diff, checkout, and adversarial-review gates passed. Pull request [#16](https://github.com/noahsabaj/arche/pull/16) merged implementation commits `7faf43a96d0e4ee4c45fb0028df0a305ffa4f5d6` and `f2c6345438acaddd89b36527c78bf890c079cdef` as `00e0fd93b5c8d57f97071a1f48939366e62e6820`; exact implementation-head run [31525196910](https://github.com/noahsabaj/arche/actions/runs/31525196910) and exact merged-main run [31526456910](https://github.com/noahsabaj/arche/actions/runs/31526456910) each passed Native Linux, Windows, physical greater-than-4-GiB source, and sparse greater-than-4-GiB executable. |
| M27-C0 | Freeze the general language and verified-generic-Core contract | The exact grammar, immutable source authority, symbolic HIR/semantic inventory, stable identity inputs, trait/ownership/effect rules, Generic Core and standalone verifier schema, CTFE budgets/accounting/receipts/projection, diagnostic ordering, and C1-C6 acceptance and corruption matrices are frozen without adding production language semantics or advancing M27-D instance/layout/AOT work. Exact-hash adversarial, mechanical, grammar, schema, and documentation-consistency passes found no remaining high or medium finding; formatting, locked debug/release workspace all-target tests, strict Clippy, dependency audit, duplicate-dependency, PowerShell Core 7.6.4, Windows PowerShell 5.1, and diff checks passed locally. Pull request [#13](https://github.com/noahsabaj/arche/pull/13) merged implementation commit `baa1d883bd8d617ca38a488797b784f7ea44840f` as `91298f7c727706660f9389f79c15ce131fae7c4d`; exact PR-head run [31287217870](https://github.com/noahsabaj/arche/actions/runs/31287217870) passed Native Linux, Windows, physical greater-than-4-GiB source, and sparse greater-than-4-GiB executable. The committed design blob's SHA-256 is `B4E8E0B8CB81E766F4E2CBE97B1FE10752955496688B1D3C3C92D833DD18A21B`. |
| M27-B | Packages, modules, resolution, and HIR | Schema-1 manifests, explicit target kinds/modules/workspaces, local-path and deterministic registry-snapshot resolution, canonical requirement-preserving `Arche.lock`, secure immutable source snapshots, package-aware resolved HIR, visibility/re-export and root-world linking, the public `arche check` path, and explicit M26 source migration diagnostics are implemented without changing the accepted `archec0` execution path. Adversarial review fixed graph/lock revalidation, registry-snapshot commitment, alias collisions, visibility widening, raw-definition target linking, public module re-exports, nested path identity, source-open TOCTOU, reserved words, and explicit-root grammar; the final audit found no remaining high or medium finding. Local formatting, locked debug/release workspace all-target tests (`293` passed, `10` intentionally ignored), strict Clippy, PowerShell Core 7.6.4 and Windows PowerShell 5.1 proof suites, dependency audit, duplicate-dependency, publication-cleanup, and diff checks passed. Pull request [#11](https://github.com/noahsabaj/arche/pull/11) merged implementation commit `cf289dda745bd09c8e33626b4ff75cd2e67d5cfb` as `fccca2712b2453bd2943f95ace1da1ccf2c13a81`; exact PR-head run [31267985870](https://github.com/noahsabaj/arche/actions/runs/31267985870) and exact merged-main run [31268239040](https://github.com/noahsabaj/arche/actions/runs/31268239040) each passed Native Linux, Windows, physical greater-than-4-GiB source, and sparse greater-than-4-GiB executable. Main protection was reconfirmed strict with those exact four required contexts. |
...
| M25-002 | Emit descriptor-sized native tables and columns | Commit `ab82e06` makes frame sizing, zeroing, table/column catalogs, and payload ranges consume `NativeWorldStoragePlan`. Demo preserves its compatible layout; Arena produces two tables and five columns across 4/8/12-byte payloads; synthetic schemas prove checked 8/16-byte alignment and non-overlap. Catalog construction is the only production path that names physical payload offsets. |
| M25-001 | Derive descriptor-generic native world storage plan | Commit `c7032f4` derives deterministic canonical component-ID keys, source-order rows, checked geometric capacities, aligned SoA payload ranges, catalog slots, and a 16-byte-aligned frame boundary from verified schemas and startup operations. Reordering declarations/component lists is deterministic; unused schemas, padding, malformed descriptor/Core agreement, and `u16` overflow are covered. |
...
| M23-002 | Materialize startup spawn rows into native storage | `cargo test --manifest-path .\bootstrap\archec0\Cargo.toml materializes_native_spawn_rows_into_archetype_storage` passed, proving generated native startup copies decoded spawn Position/Velocity payloads into stack-resident archetype storage columns, sets storage row count, capacity, and row stride, preserves startup operation validation, and keeps valid `move_system` exit `47`. |
...
| M22-002 | Iterate native descriptor table rows by count | `cargo test --manifest-path .\bootstrap\archec0\Cargo.toml iterates_native_descriptor_table_rows_by_count` passed, proving generated native descriptor record decoding walks `ECS_DESCRIPTOR_TABLE_ITERATION_ROWS` from `NATIVE_ECS_TABLE_ITERATION_CURSORS`, count-checks component/resource/system/query/schedule rows through decoded table count slots, and preserves exact descriptor field loads; `powershell -NoProfile -ExecutionPolicy Bypass -File .\tools\test.ps1` passed with the targeted proof included while valid `move_system` still exits `47`. Implementation commit: `ddf012edc`. |
| M22-001 | Define native table iteration cursor model | `cargo test --manifest-path .\bootstrap\archec0\Cargo.toml defines_native_table_iteration_cursor_model` passed, proving descriptor, startup operation, compiled schedule, and query-plan table rows are reachable through explicit native table iteration cursors over the unchanged 856-byte execution-state frame; `powershell -NoProfile -ExecutionPolicy Bypass -File .\tools\test.ps1` passed with the targeted proof included while generated `move_system` behavior remains unchanged. Implementation commit: `c7a9b7252`. |
| M21-005 | Execute compiled schedules without fixture-specific table constants | `cargo test --manifest-path .\bootstrap\archec0\Cargo.toml executes_compiled_schedules_from_table_rows_generically` passed, proving generated native compiled schedule execution is driven by `ECS_COMPILED_SCHEDULE_BUILD_ROWS` over reusable startup, decoded schedule, decoded system, compiled schedule, and query-plan rows while preserving valid `move_system` exit `47`; `powershell -NoProfile -ExecutionPolicy Bypass -File .\tools\test.ps1` passed with the targeted proof included. Implementation commit: `e91cb4cdf`. |
| M21-004 | Build native query plans from table rows generically | `cargo test --manifest-path .\bootstrap\archec0\Cargo.toml builds_native_query_plan_from_table_rows_generically` passed, proving generated native query planning is driven by `ECS_QUERY_PLAN_BUILD_ROWS` over reusable descriptor, startup, and query-plan table rows while preserving the existing planned payload address contract for compiled `Demo.Move`; `powershell -NoProfile -ExecutionPolicy Bypass -File .\tools\test.ps1` passed with the targeted proof included while valid `move_system` still exits `47`. Implementation commit: `4feab530c`. |
| M21-003 | Iterate startup operation table generically | `cargo test --manifest-path .\bootstrap\archec0\Cargo.toml iterates_native_startup_operation_table_generically` passed, proving generated native startup walks `ECS_STARTUP_OPERATION_DISPATCH_ROWS` in source order and routes resource, spawn, and run-schedule handlers through startup table slots; `powershell -NoProfile -ExecutionPolicy Bypass -File .\tools\test.ps1` passed with corruption checks for all three startup operation kinds exiting `21` while valid `move_system` still exits `47`. Implementation commit: `c6415188`. |
...
| M13-002 | Assemble component and resource descriptors from source | `cargo test --manifest-path .\bootstrap\archec0\Cargo.toml assembles_component_and_resource_descriptors_from_source` passed, proving parsed `examples\move_system.arc` assembles `Demo.Position`, `Demo.Velocity`, and `Demo.Time` runtime descriptor records with stable IDs, field offsets, sizes, and alignments; `powershell -NoProfile -ExecutionPolicy Bypass -File .\tools\test.ps1` passed with the targeted source-assembly proof included. This was assembly-only: no system/query/schedule descriptors, startup operations, `ArcheWorld` construction, descriptor registration, execution, Core changes, ELF/codegen changes, or generated executable behavior was added. Implementation commit: `7965019`. |
| M13-003 | Assemble system, query, and schedule descriptors from source | `cargo test --manifest-path .\bootstrap\archec0\Cargo.toml assembles_system_query_and_schedule_descriptors_from_source` passed, proving parsed `examples\move_system.arc` assembles `Demo.Move`, `Demo.Move.movers`, and `Demo.Main` runtime descriptor records with stable IDs and source-order query/schedule terms; `powershell -NoProfile -ExecutionPolicy Bypass -File .\tools\test.ps1` passed with the targeted source-assembly proof included. This was assembly-only: no startup operations, `ArcheWorld` construction, descriptor registration, query/schedule planning, execution, Core changes, ELF/codegen changes, or generated executable behavior was added. Implementation commit: `6ee51a6`. |
| M13-004 | Assemble startup resource payload operation | `cargo test --manifest-path .\bootstrap\archec0\Cargo.toml assembles_startup_resource_payload_operation` passed, proving parsed `examples\move_system.arc` assembles startup `resource Time { delta: 1.0 }` into a `Demo.Time` resource payload operation with exact little-endian bytes `00 00 80 3F`; `cargo test --manifest-path .\bootstrap\archec0\Cargo.toml lowers_move_system_to_core_metadata`, `cargo test --manifest-path .\bootstrap\archec0\Cargo.toml lowers_schedule_to_core_metadata`, and `powershell -NoProfile -ExecutionPolicy Bypass -File .\tools\test.ps1` passed. This was assembly-only: no `ArcheWorld` construction, descriptor registration, resource storage allocation, startup execution, real Core resource semantics, CLI behavior, ELF/codegen changes, or generated executable behavior was added. Implementation commit: `05b3d4a`. |
...
| M14-002 | Execute startup resource payload operation | `cargo test --manifest-path .\bootstrap\archec0\Cargo.toml executes_startup_resource_payload_operation` passed, proving parsed `examples\move_system.arc` can assemble startup `resource Time { delta: 1.0 }`, register descriptors into `ArcheWorld`, execute only that resource payload operation, allocate `Demo.Time` storage, store exact bytes `00 00 80 3F`, and decode `Time.delta == 1.0`; `powershell -NoProfile -ExecutionPolicy Bypass -File .\tools\test.ps1` passed with the targeted source-to-world startup resource proof included. This was resource startup execution only: no spawn execution, archetype creation, entity insertion, schedule planning, schedule execution, parser/Core changes, ELF/codegen changes, or generated executable behavior was added. Implementation commit: `addd0da`. |
| M14-003 | Execute startup spawn operation | `cargo test --manifest-path .\bootstrap\archec0\Cargo.toml executes_startup_spawn_operation` passed, proving parsed `examples\move_system.arc` can assemble startup `spawn { Position { x: 1.0, y: 2.0 } Velocity { x: 3.0, y: 4.0 } }`, register descriptors into `ArcheWorld`, execute the assembled spawn operation, allocate one entity, create the `Demo.Position + Demo.Velocity` archetype, allocate component columns, and copy exact `Position` bytes `00 00 80 3F 00 00 00 40` and `Velocity` bytes `00 00 40 40 00 00 80 40`; `powershell -NoProfile -ExecutionPolicy Bypass -File .\tools\test.ps1` passed with the targeted source-to-world startup spawn proof included. This was spawn startup execution only: no schedule planning, schedule execution, `Move` application, parser/Core changes, ELF/codegen changes, or generated executable behavior was added. Implementation commit: `4b35760`. |
...
| M18-005 | Replace bootstrap Move helper with compiled query loop proof | `cargo test --manifest-path .\bootstrap\archec0\Cargo.toml replaces_bootstrap_move_helper_with_compiled_query_loop` passed, proving codegen requires startup `run Demo.Main`, the lowered Core schedule `Demo.Main -> Demo.Move`, and the compiled `Demo.Move` query-loop body before emitting the native row scan, `mulss`, `addss`, and `Position` store sequence; `powershell -NoProfile -ExecutionPolicy Bypass -File .\tools\test.ps1` passed with the targeted unit proof, generated `move_system` now exiting `47`, corrupt metadata still exiting `16`, corrupt startup payloads still exiting `17`, and corrupt startup run schedule ID exiting `21`. This completed M18 native codegen for compiled query loops and stayed fixture-specific native proof code: no general scheduler, runtime/Core behavior change, metadata format change, or full native `ArcheWorld` was added. Implementation commit: `d89e2c8b`. |
| M19-001 | Define native ECS execution state layout | `cargo test --manifest-path .\bootstrap\archec0\Cargo.toml defines_native_ecs_execution_state_layout` passed, proving generated native ECS startup has a named 96-byte execution-state layout with descriptor count slots, startup state slots, and compiled `Demo.Move` temporary slots, and that the runtime create/destroy wrapper zeroes and restores that exact frame; `powershell -NoProfile -ExecutionPolicy Bypass -File .\tools\test.ps1` passed with the targeted layout proof included while preserving generated `move_system` exit `47` and existing corrupt metadata/payload/run-schedule proofs. This was native layout/model only: no descriptor record storage, startup dispatcher, query planner, schedule executor, metadata format change, Core/runtime behavior change, or generated-binary exit-code change was added. Implementation commit: `9b9261c8`. |
| M19-002 | Materialize native descriptor record state | `cargo test --manifest-path .\bootstrap\archec0\Cargo.toml materializes_native_descriptor_record_state` passed, proving generated native startup reads component/resource/system/query/schedule section offsets and byte lengths from the validated `ARCHEECS` directory and stores them into the expanded native execution-state frame; `powershell -NoProfile -ExecutionPolicy Bypass -File .\tools\test.ps1` passed with the targeted proof included while preserving generated `move_system` exit `47` and existing corrupt metadata/payload/run-schedule proofs. This was native descriptor-record state only: no descriptor field decoding, startup dispatcher, query planner, schedule executor, metadata format change, Core/runtime behavior change, or generated-binary exit-code change was added. Implementation commit: `b8e8519e`. |
| M19-003 | Add native startup operation dispatcher | `cargo test --manifest-path .\bootstrap\archec0\Cargo.toml dispatches_native_startup_operations` passed, proving generated rich-ECS native text stores the startup operation count, validates runtime operation kinds for resource, spawn, and run schedule records, records one dispatch count per handler, and preserves the existing resource/spawn/run handler effects; `powershell -NoProfile -ExecutionPolicy Bypass -File .\tools\test.ps1` passed with the targeted dispatcher proof and generated-binary corruption proof for unsupported startup operation kind exit `21` while preserving valid `move_system` exit `47` and existing corrupt metadata/payload/run-schedule proofs. This was a narrow native startup-operation dispatcher only: no general startup loop, query planning state, schedule executor, metadata format change, Core/runtime behavior change, or generated-binary success-code change was added. Implementation commit: `a118c716`. |
| M19-004 | Add native query planning state | `cargo test --manifest-path .\bootstrap\archec0\Cargo.toml materializes_native_query_planning_state` passed, proving generated rich-ECS native text copies the startup row count into an explicit query-plan row-count slot, stores planned `Demo.Position` and `Demo.Velocity` payload addresses, and routes compiled `Demo.Move` row scan, field math, and `Position` stores through that query-planning state; `powershell -NoProfile -ExecutionPolicy Bypass -File .\tools\test.ps1` passed with the targeted query-plan proof while preserving valid `move_system` exit `47` and existing corrupt metadata/payload/startup-operation/run-schedule proofs. This was fixture-specific native query-planning state only: no general query planner, compiled schedule state, metadata format change, Core/runtime behavior change, or generated-binary success-code change was added. Implementation commit: `0758fb2c`. |
| M19-005 | Execute compiled schedule from native state | `cargo test --manifest-path .\bootstrap\archec0\Cargo.toml executes_compiled_schedule_from_native_state` passed, proving generated rich-ECS native text loads startup `run Demo.Main` into compiled schedule state, validates `ScheduleId(0xed3d905325519b05)`, stores the Core-derived `Demo.Move` system ID `0x723b6b52df270ed5`, records one scheduled system, validates schedule state, then builds query-plan state and runs the compiled `Demo.Move` row scan/math/store path; `powershell -NoProfile -ExecutionPolicy Bypass -File .\tools\test.ps1` passed with the targeted proof while preserving valid `move_system` exit `47` and existing corrupt metadata/payload/startup-operation/run-schedule proofs. This completed M19 native ECS execution state and stayed fixture-specific compiled schedule state only: no general native scheduler, descriptor-table decoding, metadata format change, Core/runtime behavior change, or generated-binary success-code change was added. Implementation commit: `d32e7568`. |
...
M13 is complete. M14 now owns source-level runtime execution from the assembled model.
...
```text
Execute the source-derived runtime assembly model against ArcheWorld before native executable integration.
```

#### M14-001: Register assembly descriptors into ArcheWorld

Acceptance test:

```powershell
cargo test --manifest-path .\bootstrap\archec0\Cargo.toml registers_assembly_descriptors_into_world
```

Done when source-assembled component, resource, system, query, and schedule descriptors register into an `ArcheWorld` without executing startup operations.

#### M14-002: Execute startup resource payload operation

Acceptance test:

```powershell
cargo test --manifest-path .\bootstrap\archec0\Cargo.toml executes_startup_resource_payload_operation
```

Done when the assembled `Demo.Time` resource payload operation allocates storage and stores `delta = 1.0`.

#### M14-003: Execute startup spawn operation

Acceptance test:

```powershell
cargo test --manifest-path .\bootstrap\archec0\Cargo.toml executes_startup_spawn_operation
```

Done when the assembled spawn operation inserts one `Position + Velocity` entity and copies component payloads.

#### M14-004: Execute startup run schedule operation

Acceptance test:

```powershell
cargo test --manifest-path .\bootstrap\archec0\Cargo.toml executes_startup_run_schedule_operation
```

Done when the assembled `run Demo.Main` operation executes the runtime schedule plan.

#### M14-005: Execute move_system source runtime vertical slice

Acceptance test:

```powershell
cargo test --manifest-path .\bootstrap\archec0\Cargo.toml executes_move_system_source_runtime_vertical_slice
```

Done when the source-assembled runtime path updates `Position` from `Velocity * Time.delta`.
...
```text
Encode source-derived ECS descriptors and startup operations into generated native binaries before native execution consumes them.
```

#### M15-001: Define ECS metadata binary envelope

Acceptance test:

```powershell
cargo test --manifest-path .\bootstrap\archec0\Cargo.toml defines_ecs_metadata_binary_envelope
```

Done when generated ECS metadata has a deterministic magic, version, and record envelope.

#### M15-002: Encode component and resource descriptors in metadata

Acceptance test:

```powershell
cargo test --manifest-path .\bootstrap\archec0\Cargo.toml encodes_component_resource_descriptors_in_ecs_metadata
```

Done when generated ECS metadata contains `Demo.Position`, `Demo.Velocity`, and `Demo.Time` descriptor records.

#### M15-003: Encode system, query, and schedule descriptors in metadata

Acceptance test:

```powershell
cargo test --manifest-path .\bootstrap\archec0\Cargo.toml encodes_system_query_schedule_descriptors_in_ecs_metadata
```

Done when generated ECS metadata contains `Demo.Move`, `Demo.Move.movers`, and `Demo.Main` descriptor records.

#### M15-004: Encode startup operations in metadata

Acceptance test:

```powershell
cargo test --manifest-path .\bootstrap\archec0\Cargo.toml encodes_startup_operations_in_ecs_metadata
```

Done when generated ECS metadata contains the source startup resource payload, spawn, and run operation records.

#### M15-005: Decode generated ECS metadata proof
...
### M23: Native ECS World Storage Bridge

Purpose:

```text
Bridge bounded generated-native startup proof rows into explicit native archetype table storage.
```

M23 is not a heap-backed runtime world, arbitrary table allocator, metadata format change, source syntax change, event system, relation system, command buffer, object/linker split, or general scheduler. Native frame growth is acceptable only by appending slots and preserving existing offsets.

#### M23-001: Define Native Archetype Table Storage Model

Acceptance test:

```powershell
cargo test --manifest-path .\bootstrap\archec0\Cargo.toml defines_native_archetype_table_storage_model
```

Done when `codegen.rs` defines an explicit native storage model for the current `Demo.Position + Demo.Velocity` archetype table, including row count, capacity, Position column payload slots, Velocity column payload slots, and row stride for the one-row and two-row fixtures.

#### M23-002: Materialize Startup Spawn Rows Into Native Storage

Acceptance test:

```powershell
cargo test --manifest-path .\bootstrap\archec0\Cargo.toml materializes_native_spawn_rows_into_archetype_storage
```

Done when generated native startup copies decoded spawn Position/Velocity payloads into the native archetype storage model, sets storage row count from the decoded spawn count, preserves existing startup operation validation, and keeps valid `move_system` exit `47`.

#### M23-003: Build Query Plans From Native Storage State

Acceptance test:

```powershell
cargo test --manifest-path .\bootstrap\archec0\Cargo.toml builds_native_query_plan_from_archetype_storage
```

Done when native query planning derives matched/scanned row count and planned Position/Velocity payload addresses from native archetype storage state instead of direct startup payload slots.

#### M23-004: Execute Compiled Demo.Move Through Storage Columns

Acceptance test:

```powershell
cargo test --manifest-path .\bootstrap\archec0\Cargo.toml executes_compiled_move_from_native_storage_columns
```

Done when compiled `Demo.Move` reads Velocity and writes Position through query-plan addresses backed by native storage columns, validating both `move_system.arc` and `move_system_two_rows.arc` payload updates before success exit `47`.

#### M23-005: Close Native Storage Bridge Proof
...
Done when the full proof runner includes all M23 focused proofs, compiles and runs both native move fixtures through WSL with exit `47`, and WORK_LOG records M23 complete with Ready/Backlog empty.

### M24: Native ECS Storage Catalog and Descriptor-Driven Column Binding
...
```powershell
cargo test --manifest-path .\bootstrap\archec0\Cargo.toml defines_native_storage_catalog_model
```

Done when `codegen.rs` exposes bounded native storage catalog rows for the current archetype table and Position/Velocity columns without changing generated behavior.

#### M24-002: Materialize Storage Catalog Rows From Decoded Descriptors

Acceptance test:

```powershell
cargo test --manifest-path .\bootstrap\archec0\Cargo.toml materializes_native_storage_catalog_from_descriptors
```

Done when native startup fills catalog rows from decoded component/startup descriptor state for the current fixtures.

#### M24-003: Route Spawn Storage Writes Through Catalog Columns

Acceptance test:

```powershell
cargo test --manifest-path .\bootstrap\archec0\Cargo.toml materializes_spawn_rows_through_storage_catalog
```

Done when startup spawn handling writes Position/Velocity payloads through catalog-resolved storage columns instead of direct fixture slot references.

#### M24-004: Bind Query Plans Through Storage Catalog

Acceptance test:

```powershell
cargo test --manifest-path .\bootstrap\archec0\Cargo.toml builds_query_plan_through_storage_catalog
```

Done when native query planning resolves matched row count and planned payload addresses through storage catalog rows.

#### M24-005: Close Catalog-Backed Native Move Proof
...
M25 closes as a statically planned native world milestone. It computes checked geometric capacities and reserves the resulting per-program native frame, but does not add heap allocation, commands, entity lifecycle, new type families, new metadata sections, arbitrary Core execution, parallel scheduling, ML syntax, or GPU work. The two-field `f32` multiply-add execution shape is parameterized from verified Core; M26 owns arbitrary supported system bodies and schedules.
...
Arena is now a green acceptance program. Typed `i32` startup support is semantic rather than fixture-specific:

```text
cargo run --locked --manifest-path .\bootstrap\archec0\Cargo.toml -- .\examples\arena_recovery.arc --check
archec0: check passed .\examples\arena_recovery.arc
```

`Faction { id: 1.0 }` remains invalid because it is an `f32` literal for an `i32` field. M25-003 completed ordinary decimal `i32` startup literals in `0..=i32::MAX` without coercion: component values use typed Core spawn instructions, while resource values retain the checked non-Core startup path. The discovered `arena_recovery.ps1` proof now checks semantics, deterministic Core, safe publication, dynamic frame discovery, and native Linux or WSL exit `47`.
...
M25-001's planner-boundary tests used constructed verified inputs while typed Arena startup syntax was still pending. M25-003 then connected the same plan to the real checked source/Core/runtime-assembly path; no planner bypass remains in the acceptance proof.
...
- Formatting and locked debug/release all-target suites passed at 146/146. Normal Clippy passed; comparison Clippy reproduced five known diagnostics and zero new diagnostics. `cargo audit` reported no vulnerability, and `git diff --check` passed.
- Both complete local runners passed under PowerShell Core 7.6.3 and Windows PowerShell 5.1 with three dynamically discovered e2e scripts. Demo and Arena published silently and exited `47` through WSL; Arena's production frame was discovered as 1,360 bytes.
- Independent live-state observation matched the reference world byte for byte for both programs. Arena proved two archetypes, capacities `4`/`2`, three matching rows, two excluded rows, and unchanged Regeneration/Faction payloads.
...
| Boundary | Current M25 restriction | Smallest coherent M26 replacement |
|---|---|---|
| Execution package cardinality | `derive_verified_core_execution_shape` in `bootstrap/archec0/src/execution_shape.rs` requires exactly one Core/runtime schedule, one system, one resource descriptor with one startup payload, and one query descriptor. All identities and layouts are cross-checked rather than recognized by name. | Make verified Core itself the execution package and derive descriptor/runtime tables for arbitrary supported system, query, resource, and schedule counts. |
| Query/body recognition | The shape requires one read-resource parameter, one two-term mut/read query parameter, one query loop, and exactly two distinct `f32` statements of `target += source * resource`. `emit_verified_shape_query_execution` emits those lanes through planned live addresses. | Lower verified Core statements, expressions, locals, bindings, and field accesses to reference and native operations generically; delete the two-lane recognizer/template. |
| Sequential schedule dispatch | The shape accepts one sequential schedule item and inlines one verified system/query execution by descriptor identity. Parallel scheduling remains explicitly deferred. | Iterate arbitrary verified sequential schedule items, dispatch each verified Core body by system identity, and size scratch/tables from the verified program. |
| Reference execution | `execute_runtime_program_assembly_with_shape` and `execute_schedule_plan_with_shape` apply the same two-lane shape transactionally across every matching archetype. The older `execute_runtime_program_assembly`, `execute_schedule_plan`, and `execute_demo_move_system` survive only for legacy compatibility tests and are not production authority. | Interpret or lower arbitrary checker-admitted verified Core bodies in the reference world, sharing only semantic data—not expected results—with native emission; then remove the legacy adapters. |
| Terminators and proof exits | The shape rejects nonliteral or nonzero source exits. Published ECS artifacts are silent and map successful internal proof completion to `47`; malformed metadata/runtime state maps to `1`. | Make verified Core terminators source-driven in native/reference execution and keep final-state validation exclusively in external test/reference observation. |

Static startup-only storage, runtime structural mutation, entity lifecycle, and command application remain M27 concerns, not reasons to widen M26. No M26 issue is Ready, Doing, or Backlog until its acceptance programs and issue split are reviewed deliberately.
...
| Subproblem | Confidence |
|---|---:|
| `arena_recovery.arc` is structurally distinct from Demo in identities, layouts, archetype/entity counts, capacity progression, and query exclusion, yet executes without a fixture-specific production branch | 99/100 |
...
| M26 closure evidence remains preserved while M27-A, M27-B, M27-C0, and M27-C1 have exact implementation, merge, merged-main, and protected-CI evidence | 99/100 |
| Ready is empty, M27-C alone is Doing with C2 active, M27-D through M27-L are Backlog, and M27-A, M27-B, M27-C0, plus M27-C1 are recorded Done | 100/100 |
...
- `Done` contains completed M0 through M21, M22-001 through M22-005, M23-001 through M23-005, M24-001 through M24-005, M25-001 through M25-005, M26-CLOSURE, M27-A, M27-B, M27-C0, and M27-C1.
- External prerequisites remain Open in the dedicated status table and block their listed later gates without blocking in-repository M27-C work.
...
- M7 through M27-B and M27-C0-C1 are complete through their recorded scopes. M25 proves descriptor-generic storage and execution of the shared verified-Core shape across two unrelated programs; M26 closes Core-generic execution and its protected large-file gates; M27-A establishes the shared platform contract and public shell; M27-B establishes package/workspace resolution and package-aware resolved HIR; M27-C0 freezes the general language and verified-Core contract; M27-C1 implements complete retained syntax, immutable source authority, and resolved symbolic HIR/type-shape inputs without minting later semantic identities. M27-C2 is active, M27 as a whole is not complete, and M28 remains sequenced after M27.
